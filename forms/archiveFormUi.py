...
class Ui_FormArchive(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()

        self.setWindowTitle("Wood Theme - Transparent Table")
        self.setObjectName("FormArchieve")
        self.resize(1900, 1000)

        # --- ARKA PLAN RESMİNİ YÜKLEME (Background 3) ---
        self.background = QtGui.QPixmap()
        
        # 1. Öncelik: background_img3 (Düzeltilmiş picture_convert ile)
        if resources and hasattr(resources, 'background_img3'):
             self.background.loadFromData(QByteArray.fromBase64(resources.background_img3))
        # 2. Öncelik: background_img2 (Eğer eski picture_convert kullanıldıysa 3. resim buna atanmış olabilir)
        elif resources and hasattr(resources, 'background_img2'):
    ...
    def setupUI(self):
        ...
        font_label = QFont("Bodoni MT Condensed", 40)

        self.label = QtWidgets.QLabel("Aktif Olmayan Davalar")
        ...
        # --- AKTİFLEŞTİRME BUTONU ---
        self.btnActive = QtWidgets.QPushButton("Davayı Aktif Et")
        self.btnActive.setObjectName("btnActive") # Bu ID önemli, formArchive.py buradan buluyor
        self.btnActive.setFixedWidth(300)
        self.btnActive.setFixedHeight(60)
        self.btnActive.setStyleSheet("""
            QPushButton {
        ...
        self.button_layout.addWidget(self.btnActive)
        self.button_layout.addStretch()
        ...
        # --- ÖRNEK VERİLER (Veritabanı bağlanana kadar görünür) ---
        headers = [
        ...
        for row, case in enumerate(cases):
            for col, value in enumerate(case):
                ...
                if row % 2 == 0:
                    ...
                    item.setBackground(QColor(237, 227, 207, 240))
